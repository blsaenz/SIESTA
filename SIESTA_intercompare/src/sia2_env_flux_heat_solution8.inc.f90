...
!                + tmp3*(dcheat(jj) + Ed_W_temp(jj))
         ...
         if (ii .eq. ice_z) then
             ...
             r_vec(ice_z+mo) = r_vec(ice_z+mo) - DU(ii+mo)*f(mi)%t
             !DU(ii+mo) = d0_
...
!             print *,'r_vec: ',r_vec
 ...
 !            print *,'r_vec: ',r_vec
   ...
   Fc_bot = ki(ice_z+1)*(f(mi)%t-(T_next(ice_z)+T_last(ice_z))*c_5)/dth(ice_z+1) ! mean flux over sub-dt timestep
   Fc_top = ki(1)*((Ts_next+Ts_last)*c_5-(T_next(1)+T_last(1))*c_5)/dth(1)  ! mean flux over sub-dt timestep
