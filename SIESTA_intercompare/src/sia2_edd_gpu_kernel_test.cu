...
__constant__ float c1p5;
__constant__ float c2;
__constant__ float c3;
__constant__ float c4;
__constant__ float p01;
__constant__ float p001;
__constant__ float p5;
__constant__ float p75;
__constant__ float refindx;  // refractive index of sea ice (used for water also)
__constant__ float cp063;  // diffuse fresnel reflectivity from above
__constant__ float cp455;  // diffuse fresnel reflectivity from below
**/
...
//         arg      ,  // exponential argument
      ...
      //__shared__ float gauswt[8]; // gaussian weights      
      // constant arrays that won't compile as such, so they are placed here
           ...
           mu0  = fmax(g_f32_in[j],p01);   // coszen in
            ...
            { 
              ...
              if (trndir_tot > p001 ) 
              {
               ...
               j = j + blockDim.x;
               wtot    = g_f32_in[j];   // w0 in               
               j = j + blockDim.x;
               tautot  = g_f32_in[j];  // tau in
               if (k < nslyr) 
               {
                   tautot  = tautot * lda_m;
               }
...
//#pragma unroll
            ...
            g_f32_out[ki] = 33;  // layer reflectivity to direct radiation
           ki = ki + blockDim.x;
           g_f32_out[ki] = 4; // layer reflectivity to diffuse radiation from above
           ki = ki + blockDim.x;
           g_f32_out[ki] = rdif_b;  // layer reflectivity to diffuse radiation from below
           ki = ki + blockDim.x;
           g_f32_out[ki] = tdir;  // layer transmission to direct radiation (solar beam + diffuse)
           ki = ki + blockDim.x;
           g_f32_out[ki] = tdif_a;  // layer transmission to diffuse radiation from above
           ki = ki + blockDim.x;
           g_f32_out[ki] = tdif_b;  // layer transmission to diffuse radiation from below
           ki = ki + blockDim.x;
           g_f32_out[ki] = trnlay;      // solar beam transm for layer (direct beam only)                             
           ki = ki + blockDim.x;
...
/**       
           ...
           g_f32_out[ki] = 2;  // layer reflectivity to direct radiation
           ki = ki + blockDim.x;
           g_f32_out[ki] = 3; // layer reflectivity to diffuse radiation from above
           ki = ki + blockDim.x;
           g_f32_out[ki] = rdif_b;  // layer reflectivity to diffuse radiation from below
           ki = ki + blockDim.x;
           g_f32_out[ki] = tdir;  // layer transmission to direct radiation (solar beam + diffuse)
      ...
      } // end of 'is this a valid cell' test (nilyr != 0.0)
