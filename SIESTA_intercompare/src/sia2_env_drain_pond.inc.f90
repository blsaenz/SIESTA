...
! viscosity of seawater (sal=35ppt) equation: visc = 0.0015*T^2 - 0.0608*T + 1.8284 m^2/s*10e6
      ...
      if (ice(ic,mi)%pond%th .gt. 0.) then
          ...
          ! determine total drainage volume (height)
          dbvdt = (ice(ic,mi)%pond%th + ice(ic,mi)%fbh) * &
              exp( grav*ice(ic,mi)%pond%d*c_001*min_perm / (visc*ice(ic,mi)%id(sk_z)) )  ! m/s

          ! can't drain more than is currently in pond
          dbvdt = min(dbvdt*dtt_s,ice(ic,mi)%pond%th) ! m = m^3 (b/c layer is 100% brine)

          ! scale by horizontal trasport ratio
          bv_total = dbvdt*(1. - hbd_f)

          new_layer_top = bv_total
          ...
          do ii = 1,sk_z
              ...
              if (bv_total .gt. 0.) then
                  ...
                  if (bv_total .lt. 0.) then
                      dz = bv_total + th_new(jj)
                      new_layer_top = new_layer_top + dz
                      bv_total = 0.
                  else
                      dz = th_new(ii)
                      new_layer_top = new_layer_top + th_new(ii)
                  endif
              ...
              if (dz_total .lt. th_new(ii)) then
                   ...
                   do while((new_layer_top .ge. old_layer_bot) .and. (jj .lt. sk_z))
                       jj=jj+1
                       ! old thickness...
                       old_layer_bot = old_layer_bot + th_new(jj)*tmp1
                   enddo
                   old_layer_top = old_layer_bot - th_new(jj)
                   ! now jjj is OLD layer where NEW layer ii starts...
                   ...
                   do while ((old_layer_top .lt. new_layer_bot) .and. (jj .le. sk_z))
					   ...
					   heat_total = heat_total + ice(ic,mi)%t(jj)*ice(ic,mi)%bd(jjj)*cw*dz
              ...
              if (ii .lt. sk_1) then
                  ...
                  bd_mean = 1000000. + bs_mean*800.       ! find new brine
                  bd_mean = c_5*(bd_mean + ice(ic,mi)%bd(ii)) ! find mean density due to brine change
