...
!
! IMPUT VARS:
! ice(sc,ic,mi)%bv - brine vol (ppt)
! ice(sc,ic,mi)%NO3 - (mmol/m^3 in brine)
! ice(sc,ic,mi)%NH4 - (mmol/m^3 in brine)
! ice(sc,ic,mi)%PO4 - (mmol/m^3 in brine)
! ice(sc,ic,mi)%SiOH4 - (mmol/m^3 in brine)
...
! ----------------------------------------------------------------------
	  ...
	  if (dhdt_cm_s .gt. 0) then
		  ...
		  flux_sk = c_01*(ice(sc,ic,mi)%bv(sk_1)*c_001)*fvs                       ! m^3/m^2/s
	  else
		  flux_sk = 0.
	  endif
	  ...
	  do ii=1,sk_z+1
		  ...
		  dNO3(ii) = ice(sc,ic,mi)%NO3(jj)
		  dNH4(ii) = ice(sc,ic,mi)%NH4(jj)
		  dPO4(ii) = ice(sc,ic,mi)%PO4(jj)
		  dSiOH4(ii) = ice(sc,ic,mi)%SiOH4(jj)
		  dpoc(ii) = ice(sc,ic,mi)%poc(jj)
      ...
      do ii = 1,sk_z+1
		  ...
		  if (ii .eq. sk_z) then
			  ...
			  if (ice(sc,ic,mi)%bv(ii) .ge. vb_crit) then
                  ! find largest flux between brine convection, skeletal convection, and gravity drainage
				  flux_down = (1.e-5)*ice(sc,ic,mi)%bv(ii)*fvb
                  if (desal .eq. 5) then
                      b_flux(ii) = 0.
					  ! find gravity drainage
					  visc = 1.79 ! g/m/s - need equation for viscosity as a func. of temp/salinity
					  perm(ii) = (3.0e-8)*((ice(sc,ic,mi)%bv(ii)-vb_crit)*c_001)**2
					  perm(ii) =min(perm(ii),(3.0e-8)*(ice(sc,ic,mi)%bv(ii)*c_001)**3)
					  ! find pressure gradient, store in tmp1
					  if (ii .eq. 1) then
						  tmp1 = (0. - pres(ii))/ice(sc,ic,mi)%th(ii)
					  else
						  tmp1 = (pres(ii-1) - pres(ii))/ice(sc,ic,mi)%th(ii)
					  endif
					  !p_flux(ii) = perm(ii)/visc*(tmp1 + ice(sc,ic,mi)%bd(ii)*grav*ice(sc,ic,mi)%bv(ii)*c_001)
					  p_flux(ii) = perm(ii)/visc*(ice(sc,ic,mi)%bd(ii)*grav*ice(sc,ic,mi)%bv(ii)*c_001)

					  min_perm = min(min_perm,perm(ii))
                  elseif (desal .eq. 7) then
                      p_flux(ii) = 0.
                  else
                      p_flux(ii) = 0.
                      b_flux(ii) = 0.
                  ...
                  ! test -----------
                  ! find max flux
				  flux_down = max(flux_sk,flux_down,p_flux(ii))*dt_flux
				  flux_down = max(flux_down,b_flux(ii)) ! no time in b_flux - already taken care of it desal
				  Fd = flux_down/(ice(sc,ic,mi)%th(ii)*ice(sc,ic,mi)%bv(ii)*c_001)
			  else
                  Fd = 0.
			  endif
			  ...
			  dNO3(ii) = dNO3(ii) - ice(sc,ic,mi)%NO3(ii)*Fd
			  dNH4(ii) = dNH4(ii) - ice(sc,ic,mi)%NH4(ii)*Fd
			  dPO4(ii) = dPO4(ii) - ice(sc,ic,mi)%PO4(ii)*Fd
			  dSiOH4(ii) = dSiOH4(ii) - ice(sc,ic,mi)%SiOH4(ii)*Fd
			  dpoc(ii) = dpoc(ii) - ice(sc,ic,mi)%poc(ii)*Fd
              ...
              if (boundary_file .eq. 1 .and. use_pl .eq. 1) then
				  ...
				  dNO3(ii+1) = dNO3(ii+1) - ice(sc,ic,mi)%NO3(ml_z)*Fu
				  dNH4(ii+1) = dNH4(ii+1) - ice(sc,ic,mi)%NH4(ml_z)*Fu
				  dPO4(ii+1) = dPO4(ii+1) - ice(sc,ic,mi)%PO4(ml_z)*Fu
				  dSiOH4(ii+1) = dSiOH4(ii+1) - ice(sc,ic,mi)%SiOH4(ml_z)*Fu
 				  dpoc(ii+1) = dpoc(ii+1) - ice(sc,ic,mi)%poc(ml_z)*Fu
             endif
		  ...
		  elseif (ii .eq. sk_z+1) then
              ...
              if (use_pl .eq. 1) then
				  ...
				  ! linear eqn constant - only for explicit euler
				  dNO3(ii) = dNO3(ii) + Feddy*(f(mi)%no3 - ice(sc,ic,mi)%NO3(ml_z))
				  dNH4(ii) = dNH4(ii) + Feddy*(f(mi)%nh4 - ice(sc,ic,mi)%NH4(ml_z))
				  dPO4(ii) = dPO4(ii) + Feddy*(f(mi)%po4 - ice(sc,ic,mi)%PO4(ml_z))
				  dSiOH4(ii) = dSiOH4(ii) + Feddy*(f(mi)%sioh4 - ice(sc,ic,mi)%SiOH4(ml_z))
				  dpoc(ii) = dpoc(ii) + Feddy*(f(mi)%poc - ice(sc,ic,mi)%poc(ml_z))
			  ...
			  if (ice(sc,ic,mi)%bv(ii) .ge. vb_crit .and. ice(sc,ic,mi)%bv(ii+1) .ge. vb_crit) then

                  ! find inter-layer geomentric mean brine volume, and flux distance
				  th_down = ice(sc,ic,mi)%th(ii) + ice(sc,ic,mi)%th(ii+1)
				  bv_down = ice(sc,ic,mi)%bv(ii)*ice(sc,ic,mi)%bv(ii+1)/ &
				    (ice(sc,ic,mi)%th(ii)*ice(sc,ic,mi)%bv(ii+1) + &
				     ice(sc,ic,mi)%th(ii+1)*ice(sc,ic,mi)%bv(ii))*th_down
				  th_down = th_down*c_5
                  ...
                  ! test -----------
                  !flux_down = (1.e-5)*51.*fvb  ! convert fvb to m^3/m^2/s, times brine fraction
                  ! test------------
				  ...
				  flux_down = max(flux_down,p_flux(ii))*dt_flux
				  flux_down = max(flux_down,b_flux(ii)) ! no time in b_flux - already taken care of it desal

				  ! calc matrix coefficients
				  Fd = flux_down/(ice(sc,ic,mi)%th(ii)*ice(sc,ic,mi)%bv(ii)*c_001)
                  Fu = flux_down/(ice(sc,ic,mi)%th(ii+1)*ice(sc,ic,mi)%bv(ii+1)*c_001)
				  ...
				  dNO3(ii+1) = dNO3(ii+1) - ice(sc,ic,mi)%NO3(ii+1)*Fu
				  dNH4(ii+1) = dNH4(ii+1) - ice(sc,ic,mi)%NH4(ii+1)*Fu
				  dPO4(ii+1) = dPO4(ii+1) - ice(sc,ic,mi)%PO4(ii+1)*Fu
				  dSiOH4(ii+1) = dSiOH4(ii+1) - ice(sc,ic,mi)%SiOH4(ii+1)*Fu
				  dpoc(ii+1) = dpoc(ii+1) - ice(sc,ic,mi)%poc(ii+1)*Fu
...
!          if (ii .eq. sk_z) then
!              ice(sc,ic,mi)%fbv(ii) = ice(sc,ic,mi)%fbv(ii) + flux_down
!          endif
...
!           if (ii .ge. sk_1) then
!			   bv_mean = c_001*ice(sc,ic,mi)%bv(ii)
!			   tmp1 = dno3(ii)*bv_mean
!			   if (tmp1 .ge. f(mi)%no3) then
!							  testvar = -1
!							  testvar = sqrt(testvar)
!			   endif
!			   tmp2 = ice(sc,ic,mi)%NO3(ii)*bv_mean
!			   if (tmp2 .ge. f(mi)%no3) then
!							  testvar = -1
!							  testvar = sqrt(testvar)
!			   endif
!            endif
		  ...
		  ice(sc,ic,mi)%NO3(jj) = dNO3(ii)
		  ice(sc,ic,mi)%NH4(jj) = dNH4(ii)
		  ice(sc,ic,mi)%PO4(jj) = dPO4(ii)
		  ice(sc,ic,mi)%SiOH4(jj) = dSiOH4(ii)
		  ice(sc,ic,mi)%poc(jj) = dpoc(ii)

	      ! test flux
		  if (ice(sc,ic,mi)%NO3(jj) .lt. 0.) then
			  ice(sc,ic,mi)%NO3(jj) = 0.
		  endif

		  if (ice(sc,ic,mi)%NH4(jj) .lt. 0.) then
			  !print *, 'NH4 less than zero',ii,ice(sc,ic,mi)%NH4(ii),i,j
			  ice(sc,ic,mi)%NH4(jj) = 0.
		  endif

		  if (ice(sc,ic,mi)%PO4(jj) .lt. 0.) then
			  ice(sc,ic,mi)%PO4(jj) = 0.
		  endif

		  if (ice(sc,ic,mi)%SiOH4(jj) .lt. 0.) then
			  ice(sc,ic,mi)%SiOH4(jj) = 0.
		  endif

		  if (ice(sc,ic,mi)%poc(jj) .lt. 0.) then
			  ice(sc,ic,mi)%poc(jj) = 0.
		  endif
