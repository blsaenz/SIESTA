...
!      if (ice(sc,ic,mi)%id(int_z) .ge. 0.2) then
          ...
          do ii=1,int_z
		      ...
		      dbvdt = 0.d0
		      F0 = 0.d0
              bv_mean = ice(sc,ic,mi)%bv(ii)  ! ppt
              dz = ice(sc,ic,mi)%bv(ii)*c_001*ice(sc,ic,mi)%th(ii) ! layer brine volume (m^3)
...
!                .and. (ice(sc,ic,mi)%bs(ii) .gt. f(mi)%s)) &
!                .or. ((ice(sc,ic,mi)%bced(ii) .eq. 1) .and. (bv_mean .ge. vb_crit)) &
                )) then

                  if (ice(sc,ic,mi)%bs(ii) .gt. f(mi)%s) then
                      ...
                      if (ii .eq. 1) then
!                          if (z_snow .gt. 0) then
!                              T_grad = (ice(sc,ic,mi)%snow%t(z_snow) - ice(sc,ic,mi)%t(ii+1)) / &
!                                 (c_5*(ice(sc,ic,mi)%th(ii+1) + ice(sc,ic,mi)%th(ii) + ice(sc,ic,mi)%snow%t(z_snow)))
!                          else
                              T_grad = (ice(sc,ic,mi)%t(ii) - ice(sc,ic,mi)%t(ii+1)) / &
                                  (c_5*(ice(sc,ic,mi)%th(ii+1) + ice(sc,ic,mi)%th(ii)))
!                          endif

                      elseif (ii .eq. int_z) then
                           T_grad = (ice(sc,ic,mi)%t(ii-1) - f(mi)%t) / &
                               (c_5*ice(sc,ic,mi)%th(ii-1) + ice(sc,ic,mi)%th(ii))
                      else
                           T_grad = (ice(sc,ic,mi)%t(ii-1) - ice(sc,ic,mi)%t(ii+1)) / &
                               (c_5*(ice(sc,ic,mi)%th(ii-1) + ice(sc,ic,mi)%th(ii+1)) + ice(sc,ic,mi)%th(ii))
                      endif

                      ice(sc,ic,mi)%tgrad(ii) = T_grad
                      ...
                      !ice(sc,ic,mi)%dsdt(ii) = -1.*T_grad*c_01* & ! 0.01 factor is to convert from 1/m to 1/cm
                      !   (1.68E-5 - 3.37E-7*bv_mean)

											! find latent heat associated with cox/week gravity drainage
											if (dsdt1 .lt. 0.d0) then
												...
												dcheat(ii) = abs(dsdt1)/s_new(ii)*bv_mean*c_001*ice(sc,ic,mi)%th(ii)*Lf*(1.d0-bb_f)
                      endif
                      ...
                      if (ii .eq. 1) then
                          if (z_snow .gt. 1) then
                             tmp1 = (ice(sc,ic,mi)%t(1) - ice(sc,ic,mi)%snow%t(1))/dth(ice_1)
                          else
                             tmp1 = (ice(sc,ic,mi)%t(1) - ice(sc,ic,mi)%snow%ts)/dth(ice_1)
                          endif
                          tmp2 = (ice(sc,ic,mi)%t(1) - ice(sc,ic,mi)%t(2))/dth(ice_1+1)
                      else
                          tmp1 = (ice(sc,ic,mi)%t(ii) - ice(sc,ic,mi)%t(ii-1))/dth(z_snow+ii)
                          tmp2 = (ice(sc,ic,mi)%t(ii) - ice(sc,ic,mi)%t(ii+1))/dth(z_snow+ii+1)
                      endif
...
!                      if (iii .ge. 1) then
                          ...
                          tmp1 = F0/(1.d0-bb_f) ! overcompensate F0 to simulate bubble fraction
!                          if (ii .gt. 1) then
!                              tmp1 = max(tmp1,ice(sc,ic,mi)%f0(ii))
!                          endif

                          ! F0 calc from temp gradient - moved outside loop for testing...
                          ! --------------------------------------------
                          !ki_down = (d_new(ii)/IceD)*(c0-0.011*ice(sc,ic,mi)%t(ii) + &
                          !  0.09*s_new(ii)/ice(sc,ic,mi)%t(ii) - (d_new(ii)-IceD)/1.e6)
                          !F0 = -1.*T_grad*ki_down

                          ! F0 = heat_last(ii)*ice(sc,ic,mi)%th(ii)/dtt_s
                          ...
                          ! dhdt = 9.873e-12*tmp1**3 - 6.621e-10*tmp1**2 + 4.474e-7*tmp1  ! (cm/s) dhdt vs. F0 regression at T=-1.8 freezing temp, petrich keff
                          !dhdt = 9.988e-12*F0**3 - 6.714e-10*F0**2 + 4.484e-7*F0  ! (cm/s) dhdt vs. F0 regression at T=-1.6 freezing temp, petrich keff

!                          if (F0 .le. 5.364) then
!                              dhdt = 2.d-6
                          !else
                          if (tmp1 .ge. 210.d-0) then
                              dhdt = 2.d-4
                          elseif (tmp1 .le. 57.316) then
                              dhdt = 1.449d-9*tmp1**2 + 3.524d-7*tmp1 + 5.212d-8
                          else
                              dhdt = 1.942d-13*tmp1**4 - 6.d-11*tmp1**3 + 7.387d-9*tmp1**2 + 1.755d-7*tmp1  ! (cm/s) dhdt vs. F0 regression at T=-1.88 freezing temp, petrich keff
                          endif

!                      else
                          ! use dhdt from last timestep if this is 1st sub-dy timestep and there is no heat_diff/t-diff yet
!                          dhdt = ice(sc,ic,mi)%dhdt_conv(ii)
!                      endif

                      ! record instaneous flux-based desal vars
                      ice(sc,ic,mi)%dhdt_conv(ii) = dhdt
                      ice(sc,ic,mi)%f0(ii) = F0
                      ...
                      if (dhdt .gt. 0.) then
                          ...
                          ! ice(sc,ic,mi)%dsdt3(ii) = -1.*dhdt*c_01*ice(sc,ic,mi)%bs(ii)*(1.-keff)/ice(sc,ic,mi)%th(ii)
                           ice(sc,ic,mi)%dsdt3(ii) = F0/Lf*ice(sc,ic,mi)%bs(ii)/918000.d0/ice(sc,ic,mi)%th(ii)
...
!                      if (bv_mean .ge. bv_conv .and. dhdt .gt. 0.) then
                      if (bv_mean .ge. bv_conv .and. dhdt .gt. 0.) then
                          ...
                          if (s_new(ii) .gt. keff*ice(sc,ic,mi)%bs(ii)) then
                              ...
                              !dcheat(ii) = heat_last(ii)*ice(sc,ic,mi)%th(ii)/dtt_s
                              dcheat(ii) = F0

                              tmp4 = dhdt*c_01*ice(sc,ic,mi)%bs(ii)*(1.-keff)/ice(sc,ic,mi)%bs(ii)*334.*918000 ! watts/m^2
                              ! print *,ii,'-- ice frozen: ',tmp4,'heat flux: ',F0
                              ...
                              ! do agressive desal
                              dsdt3 = -1.*dtt_s*F0/Lf*ice(sc,ic,mi)%bs(ii)/918000.d0/ice(sc,ic,mi)%th(ii)/(1.d0-bb_f)
                              ! using modifed dhdt/tmp4
                              !dsdt3 = -1.*dtt_s*tmp4/Lf*ice(sc,ic,mi)%bs(ii)/918000.d0/ice(sc,ic,mi)%th(ii)

                              ! using modifed dhdt/tmp4
                              ! dsdt3 = -1.*dhdt*c_01*ice(sc,ic,mi)%bs(ii)*(1.-keff)/ice(sc,ic,mi)%th(ii)

                              ! scale dsdt according to bv^3
!                              tmp1 = (ice(sc,ic,mi)%bv(ii)-bv_conv)   ! difference
...
!	              endif
			  ...
			  if (dsdt1 .lt. 0.d0) then
                  ...
                  tmp2 = ice(sc,ic,mi)%bs(ii+1) - ice(sc,ic,mi)%bs(ii)
!                  tmp2 = tmp1
                  tmp1 = min(-2.,tmp1)
                  tmp2 = min(-2.,tmp2)
...
!                  if(ice(sc,ic,mi)%bced(ii) .eq. 1) then
                  if (ice(sc,ic,mi)%bv(ii) .ge. bv_conv) then
                      ! dilute the original bs with seawater - assume direct connection
                      ! with seawater
                      dbvdt = ((ice(sc,ic,mi)%bs(ii)*bv_mean + dsdt1)/bv_mean - &
                          ice(sc,ic,mi)%bs(ii))/(tmp1)
                  else
                      ! dilute the original bs to bs_dilute with brine below
                      dbvdt = ((ice(sc,ic,mi)%bs(ii)*bv_mean + dsdt1)/bv_mean - &
                          ice(sc,ic,mi)%bs(ii))/(tmp2)
                  endif

                  ! convert to m^3/timestep
                  dbvdt = dbvdt*ice(sc,ic,mi)%th(ii)*bv_mean

				  ! old dbvdt calc
!				  S_mean = s_new(ii) + dsdt1
!			      bs_dilute = 1000.*ice(sc,ic,mi)%d(ii)*S_mean/ &
!					  (ice(sc,ic,mi)%bd(ii)*ice(sc,ic,mi)%bv(ii))
!				  bs_dilute = ice(sc,ic,mi)%bs(ii)*(s_mean/s_new(ii))   ! effectively the same thing as above
!                 if (ice(sc,ic,mi)%bv(ii) .ge. bv_conv) then
                      ! dilute the original bs to bs_dilute with seawater - assume direct connection
                      ! with seawater
!                      dbvdt = 0.918*ice(sc,ic,mi)%bv(ii)*ice(sc,ic,mi)%th(ii)*c_001* &
!					      (bs_dilute - ice(sc,ic,mi)%bs(ii))/(f(mi)%s - bs_dilute)
                      ! replacement dilution
!                   else
                      ! dilute the original bs to bs_dilute with brine below
!                      dbvdt = 0.918*ice(sc,ic,mi)%bv(ii)*ice(sc,ic,mi)%th(ii)*c_001* &
!                        (bs_dilute - ice(sc,ic,mi)%bs(ii))/(ice(sc,ic,mi)%bs(ii+1) - bs_dilute) ! m^3/m^2/s
!                      dbvdt = 0.918*ice(sc,ic,mi)%bv(ii)*ice(sc,ic,mi)%th(ii)*c_001* &
!                        (bs_dilute - ice(sc,ic,mi)%bs(ii))/(f(mi)%s - bs_dilute) ! m^3/m^2/s
!                  endif



		           !fvs = -9.652e-9 + 0.387*dhdt - 1029.1*dhdt**2  ! cm^3/cm^2/s - from arrigo basic bodel
		           !dbvdt = c_01*(ice(sc,ic,mi)%bv(ii)*c_001)*fvs                       ! m^3/m^2/s
                  ...
                  if (ice(sc,ic,mi)%bv(ii) .ge. bv_conv .and. dhdt .gt. 0.) then

                      ! available convective heat
                      tmp1 = dbvdt*cw*(f(mi)%t-ice(sc,ic,mi)%t(ii))*f(mi)%d ! J/m^3

                      ! heat capacity of layer --> seawater temp
                      T_melt = mu*s_new(ii)
			          tmp2 = -1.*d_new(ii)*(c0*(f(mi)%t - ice(sc,ic,mi)%t(ii)) &
				         - Lf*mu*s_new(ii)*(1./f(mi)%t - 1./ice(sc,ic,mi)%t(ii))) &
				         * ice(sc,ic,mi)%th(ii) ! J/m^3 - not J/m^2

				      !print *,'F0: ',F0,'heat brine: ',tmp1, 'layer_heat: ',tmp2

!				      dcheat(ii) = min(tmp1,tmp2)/dtt_s
				      !heat_total = heat_total + tmp1

				  else

                      ! available convective heat
                      tmp1 = dbvdt*cw*(ice(sc,ic,mi)%t(ii+1)-ice(sc,ic,mi)%t(ii))*f(mi)%d

                      ! heat capacity of layer --> seawater temp
                      T_melt = mu*s_new(ii)
			          tmp2 = -1.*d_new(ii)*(c0*(ice(sc,ic,mi)%t(ii+1) - ice(sc,ic,mi)%t(ii)) &
				         - Lf*mu*s_new(ii)*(1./ice(sc,ic,mi)%t(ii+1) - 1./ice(sc,ic,mi)%t(ii))) &
				         * ice(sc,ic,mi)%th(ii) ! j/m^3 - not j/m^2
...
!                  if(ice(sc,ic,mi)%bced(ii) .eq. 1) then
                  if (ice(sc,ic,mi)%bv(ii) .ge. bv_conv) then
                      ...
                      dbvdt = dsdt2*ice(sc,ic,mi)%th(ii)/f(mi)%s
                  else
                      ! add volume of brine needed to reach new salinity
                      dbvdt = dsdt2*ice(sc,ic,mi)%th(ii)/ice(sc,ic,mi)%bs(ii+1)
                  endif
              ...
              if (ice(sc,ic,mi)%bv(ii) .ge. bv_conv) then
                  ...
                  d_total = d_total + dz*ice(sc,ic,mi)%bd(ii)
                  heat_brine = heat_brine + dz*cw*ice(sc,ic,mi)%t(ii)*ice(sc,ic,mi)%bd(ii)
              ...
              if ((ice(sc,ic,mi)%bv(ii) .lt. bv_conv) .or. (ii .eq. int_z)) then
                  ...
                  if ((ii .eq. int_z) .and. ice(sc,ic,mi)%bv(ii) .ge. bv_conv) then
                      mo = 1
                  else
                      mo = 0
...
!-------------
! for testing convective nut resupply only
!                   bv_flux = 0.d0
!------------
                      tmp1 = 1.d0/(bv_total)
                      ...
                      ! find mean brine temp for calc mixed-brine heat
                      d_total = (d_total*tmp2 + bv_flux*f(mi)%d)*tmp1  ! g/m^3
                      heat_brine = (heat_brine*tmp2 * bv_flux*cw*f(mi)%d*f(mi)%t)*tmp1 ! J/m-3
                      !t_mean = heat_brine/(d_total*cw)
...
!                          endif
                          ...
                          ! heat capacity of layer --> seawater temp
                          !T_melt = mu*s_new(jj)
                          !tmp2 = -1.*d_new(jj)*(c0*(f(mi)%t - ice(sc,ic,mi)%t(jj)) &
                          !   - Lf*mu*s_new(jj)*(1./f(mi)%t - 1./ice(sc,ic,mi)%t(jj))) &
                          !   * ice(sc,ic,mi)%th(jj) ! j/m^3 - not j/m^2

                          ! heat difference between mixed brine and current brine
                          tmp3 = heat_brine - &
                                 ice(sc,ic,mi)%t(jj)*cw*ice(sc,ic,mi)%bd(jj) ! J/m^3

                          !dcheat(jj) = tmp3*ice(sc,ic,mi)%bv(jj)*c_001* &
                          !    ice(sc,ic,mi)%th(jj)/dtt_s ! W
          ...
          if (integr_valid) then
              ...
              do ii=1,int_z
                  ...
                  ! record vars
                  ice(sc,ic,mi)%dsdt(ii) = ice(sc,ic,mi)%dsdt(ii) + dsdt_cw(ii)
                  ice(sc,ic,mi)%fbv(ii) = ice(sc,ic,mi)%fbv(ii) + h_flux(ii)
              ...
              do ii=sk_1,sk_z
                  ...
                  tmp1 = ice(sc,ic,mi)%bv(ii)*c_001*ice(sc,ic,mi)%th(ii)*0.3
                  do while (b_flux(ii)/dble(bfrf) .ge. tmp1)
                      bfrf = bfrf*2
...
!          print *,'bced: ',ice(sc,ic,mi)%bced
!          print *,'dsdt: ',dsdt
!          print *,'dheat: ',dheat
!          print *,'no3: ',ice(sc,ic,mi)%no3
