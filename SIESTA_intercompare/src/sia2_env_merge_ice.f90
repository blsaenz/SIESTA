...
! merges ice conservatively from two categories
      ...
      subroutine sia2_env_merge_ice(sc_in,ic_in,sc_out,ic_out,mi,ice,pur,f,pl)
		  ...
		  !----------------------------------------------------
          !indices
          integer :: i,j,ii,jj,kk,jjj,iii,int_z,sk_1,sk_z,sc,ic,i_ease,j_ease,z_old
          ...
          double precision :: mf_depth,melt_drain,c_gl_copy,sk_gl
          ...
          ! airtemp1c not used...except in melt/drain but shouldn't enter that here
          ! snowfall_d shouldn't be needed either but small floating point weirdness requires it
