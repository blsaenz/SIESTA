...
! ======================================================================
      ...
      v_f, stations_mdh,stations_mdv)
          ...
          real :: d_real
          double precision :: u,v,flag,tmp1,tmp2,p_h2o,p_h2o_sat, &
          	row_air,esat_w,esat_i,h_w,h_i,t_ice,r_o_r,alph,dewp
...
!$OMP d_real,month,u,v,flag,tmp1,tmp2,p_h2o,p_h2o_sat,row_air, &
!$OMP esat_w,esat_i,h_w,h_i,alph,dewp)
...
!$OMP DO SCHEDULE(DYNAMIC,chunk_size)
          ...
          do mi = 1,tcells
              ...
              if (boundary_file .eq. 1 .and. v_f%rhum .ne. c9999) then
              ...
              elseif (atm_f .ge. 1) then
                  ! get dewpoint
                  if (atm_f .eq. 1) then
                      dewp = ec_f%ecmwf_interp%dpt(i_ec,j_ec) ! K
                  elseif (atm_f .eq. 2) then
                      dewp = eci_f%ecmwf_interp%dpt(i_eci,j_eci) ! K
                  endif

									! find alpha for partitioning saturation humidity
									alph = 1.d0;
									if (f(mi)%at .le. t_ice) then
											alph = 0.d0;
									elseif (f(mi)%at .gt. 273.16) then
											alph = ((f(mi)%at-t_ice)/(273.16-t_ice))**2;
									endif

									! find saturation humidity ECMWF style
									esat_w = 611.2 * &
										exp(17.502 * ((f(mi)%at - 273.16)/(f(mi)%at - 32.19))) ! over water
									esat_i = 611.2 * &
										exp(22.587 * ((f(mi)%at - 273.16)/(f(mi)%at + 20.7))) ! over ice
									h_w = (r_o_r*esat_w)/(f(mi)%p - (1.d0-r_o_r)*esat_w);
									h_i = (r_o_r*esat_i)/(f(mi)%p - (1.d0-r_o_r)*esat_i);
									f(mi)%rhum = alph*h_w + (1.d0-alph)*h_i ! store h_sat in rhum

									! find specific humidity from dewpoint ECMWF style
									esat_w = 611.2 * &
										exp(17.502 * ((dewp - 273.16)/(dewp - 32.19))) ! over water
									esat_i = 611.2 * &
										exp(22.587 * ((dewp - 273.16)/(dewp + 20.7))) ! over ice
									h_w = (r_o_r*esat_w)/(f(mi)%p - (1.d0-r_o_r)*esat_w);
									h_i = (r_o_r*esat_i)/(f(mi)%p - (1.d0-r_o_r)*esat_i);
									f(mi)%h = alph*h_w + (1.d0-alph)*h_i ! store h_sat in rhum

									! find relative humidity
									f(mi)%rhum = f(mi)%h / f(mi)%rhum
              ...
              if (boundary_file .eq. 1) then
                  ...
                  if (v_f%sh .ne. c9999) then
                      ...
                      if (f(mi)%sh_interp .eq. c9999) then
                          print *,'Invalid snow depth in boundary_file mode.  Check boundary data and snow_model compatibility.'
                          call exit(0)
                      endif
                  endif
 			      f(mi)%ic_interp = 1.
